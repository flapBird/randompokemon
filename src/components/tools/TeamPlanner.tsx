...
export function TeamPlanner({ initialPokemon = "" }: { initialPokemon?: string }) {
  ...
  return <section className="tool-shell team-planner-tool">
    <div className="planner-builder"><div className="planner-heading"><div><span className="eyebrow">TEAM BUILDER</span><h2>Your team</h2></div><strong>{team.length}/6</strong></div>
      <p className="planner-intro">{team.length ? "Keep choosing specific Pokémon, or let the planner complete the open slots." : "Start with the Pokémon you want to use, or fill all six slots with a random team. Empty slots keep the plan yours."}</p>
      ...
      <div className="planner-actions"><button onClick={() => fill(false)} disabled={team.length >= 6 || loading}>{team.length ? "Randomize Remaining" : "Fill 6 Random Pokémon"}</button><button className="generate-button" onClick={() => fill(true)} disabled={team.length >= 6 || loading}>{team.length ? "Improve Coverage" : "Build a Balanced Team"}</button><button onClick={() => setTeamSlugs([])} disabled={!team.length}>Clear Team</button></div>
